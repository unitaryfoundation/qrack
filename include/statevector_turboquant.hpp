...
//   QRACK_TURBO_BLOCK_SIZE  — amplitudes per block (default 64, must be power of 2)
...
struct TurboBlock {
    ...
    static constexpr size_t NWORDS = (2U * D * BITS + 63U) / 64U;
    std::array<uint64_t, (2U * QRACK_TURBO_BLOCK_SIZE * QRACK_TURBO_BITS + 63U) / 64U> packed;
...
public:
    StateVectorTurboQuant(bitCapIntOcl cap)
        : StateVector(cap)
