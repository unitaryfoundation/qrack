...
Environment variable `QRACK_SPARSE_TRUNCATION_THRESHOLD` controls the threshold of probability below which amplitudes are truncated during any `Compose()`-type operation under `QUnit` is sparse (CPU) simulation is in use. During a `Compose()` operation, if an amplitude multiplies out to a value with a norm below this threshold, it will be truncated to `0` (to save memory), and the first-principles, worst-case fidelity estimate will be updated to reflect the lost fidelity. To automatically tune this parameter for random circuit sampling (RCS), you can instead set environment variable `QRACK_RCS_Z` to the closest integer to average qubit connectivity (like 4 for a square chip grid layout) or any non-numeric value for full-connectivity cases. (This sets `QRACK_SPARSE_TRUNCATION_THRESHOLD` to `1.0 / (QRACK_RCS_Z * 2.0 ^ n)` for `n` count of qubits.)
