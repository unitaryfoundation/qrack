    ...
    , sparse_thresh(_qrack_sparse_thresh)
{
    ...
    if (phaseFac == CMPLX_DEFAULT_ARG) {
        stateVec->write((size_t)(uint64_t)initState, GetNonunitaryPhase());
    } else {
        stateVec->write((size_t)(uint64_t)initState, phaseFac);
    }
}
...
void QEngineCPU::GetAmplitudePage(complex* pagePtr, bitCapIntOcl offset, bitCapIntOcl length)
{
    ...
    if (stateVec) {
        stateVec->copy_out(pagePtr, (size_t)offset, (size_t)length);
    } else {
        par_for(0, length, [&](const size_t& lcv, const unsigned& cpu) { pagePtr[lcv] = ZERO_CMPLX; });
    }
}
void QEngineCPU::SetAmplitudePage(const complex* pagePtr, bitCapIntOcl offset, bitCapIntOcl length)
{
    ...
    stateVec->copy_in(pagePtr, (size_t)offset, (size_t)length);
    ...
    QEnginePtr pageEnginePtr, bitCapIntOcl srcOffset, bitCapIntOcl dstOffset, bitCapIntOcl length)
{
    ...
    stateVec->copy_in(oStateVec, (size_t)srcOffset, (size_t)dstOffset, (size_t)length);
...
complex QEngineCPU::GetAmplitude(const bitCapInt& perm)
{
    ...
    return stateVec->read((size_t)(uint64_t)perm);
}
...
void QEngineCPU::SetAmplitude(const bitCapInt& perm, const complex& amp)
{
    ...
    if (runningNorm != REAL1_DEFAULT_ARG) {
        runningNorm += norm(amp) - norm(stateVec->read((size_t)(uint64_t)perm));
    }
    ...
    stateVec->write((size_t)(uint64_t)perm, amp);
}
...
void QEngineCPU::SetPermutation(const bitCapInt& perm, const complex& phaseFac)
{
    ...
    if (phaseFac == CMPLX_DEFAULT_ARG) {
        ...
        stateVec->write((size_t)(uint64_t)perm, phase);
    } else {
        real1 nrm = abs(phaseFac);
        stateVec->write((size_t)(uint64_t)perm, phaseFac / nrm);
    }
...
{
    if (!stateVec) {
        return par_for(0, maxQPowerOcl, [&](const size_t& lcv, const unsigned& cpu) { outputState[lcv] = ZERO_CMPLX; });
    }
...
{
    if (!stateVec) {
        return par_for(0, maxQPowerOcl, [&](const size_t& lcv, const unsigned& cpu) { outputProbs[lcv] = ZERO_R1; });
    }
...
#define NORM_THRESH_KERNEL(o1, o2, fn, bci)                                                                            \
    [&](const bci& lcv, const unsigned& cpu) {                                                                         \
...
#define NORM_CALC_KERNEL(o1, o2, fn, bci)                                                                              \
    [&](const bci& lcv, const unsigned& cpu) {                                                                         \
        complex2 qubit = stateVec->read2(lcv + o1, lcv + o2);                                                          \
        qubit = fn;                                                                                                    \
    ...
    const bitCapInt* qPowsSorted, bool doCalcNorm, real1_f nrm_thresh)
{
    ...
    if (stateVec->is_sparse()) {
        return Apply2x2Sparse(offset1, offset2, matrix, bitCount, qPowsSorted, doCalcNorm, nrm_thresh);
    }
        ...
        [this, mtrxS, qPowersSorted, offset1, offset2, doCalcNorm, doApplyNorm, nrm, nrm_thresh] {
            ...
            if (!doCalcNorm) {
                ...
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_THRESH_KERNEL(offset1, offset2, nrm * mtrxPhase * qubit, size_t);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_THRESH_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, size_t);
                    } else {
                        fn = NORM_THRESH_KERNEL(offset1, offset2,
                            matrixMul(nrm, mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), size_t);
                    }
                } else {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_THRESH_KERNEL(offset1, offset2, mtrxPhase * qubit, size_t);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_THRESH_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, size_t);
                    } else {
                        fn = NORM_THRESH_KERNEL(offset1, offset2,
                            matrixMul(mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), size_t);
                    }
                }
            } else {
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_CALC_KERNEL(offset1, offset2, nrm * mtrxPhase * qubit, size_t);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_CALC_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, size_t);
                    } else {
                    ...
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_CALC_KERNEL(offset1, offset2, mtrxPhase * qubit, size_t);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_CALC_KERNEL(offset2, offset1, mtrxPhase * qubit, size_t);
                    } else {
                        fn = NORM_CALC_KERNEL(offset1, offset2,
                            matrixMul(mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), size_t);
                    }
    ...
    Dispatch(maxQPowerOcl >> bitCount,
        [this, mtrxS, qPowersSorted, offset1, offset2, doCalcNorm, doApplyNorm, nrm, nrm_thresh, bitCount] {
            ...
            if (!doCalcNorm) {
                ...
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_THRESH_KERNEL(offset1, offset2, nrm * mtrxPhase * qubit, bitCapInt);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_THRESH_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, bitCapInt);
                    } else {
                        fn = NORM_THRESH_KERNEL(offset1, offset2,
                            matrixMul(nrm, mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), bitCapInt);
                    }
                } else {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_THRESH_KERNEL(offset1, offset2, mtrxPhase * qubit, bitCapInt);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_THRESH_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, bitCapInt);
                    } else {
                        fn = NORM_THRESH_KERNEL(offset1, offset2,
                            matrixMul(mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), bitCapInt);
                    }
                }
            } else {
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_CALC_KERNEL(offset1, offset2, nrm * mtrxPhase * qubit, bitCapInt);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_CALC_KERNEL(offset2, offset1, nrm * mtrxPhase * qubit, bitCapInt);
                    } else {
                        fn = NORM_CALC_KERNEL(offset1, offset2,
                            matrixMul(nrm, mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), bitCapInt);
                    }
                    ...
                    if (IS_NORM_0(mtrx[1U]) && IS_NORM_0(mtrx[2U])) {
                        fn = NORM_CALC_KERNEL(offset1, offset2, mtrxPhase * qubit, bitCapInt);
                    } else if (IS_NORM_0(mtrx[0U]) && IS_NORM_0(mtrx[3U])) {
                        fn = NORM_CALC_KERNEL(offset2, offset1, mtrxPhase * qubit, bitCapInt);
                    } else {
                        fn = NORM_CALC_KERNEL(offset1, offset2,
                            matrixMul(mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit), bitCapInt);
                    }
...
#define NORM_THRESH_KERNEL(fn1, fn2, bci)                                                                              \
    [&](const bitCapInt& lcv, const unsigned& cpu) {                                                                   \
        complex qubit[2U];                                                                                             \
                                                                                                                       \
...
#define NORM_CALC_KERNEL(fn1, fn2, bci)                                                                                \
    [&](const bitCapInt& lcv, const unsigned& cpu) {                                                                   \
        complex qubit[2U];                                                                                             \
                                                                                                                       \
        const complex Y0 = stateVec->read(lcv + offset1);                                                              \
    ...
    Dispatch(maxQPowerOcl >> bitCount,
        [this, mtrxS, qPowersSorted, offset1, offset2, doCalcNorm, doApplyNorm, nrm, nrm_thresh] {
            ...
            if (!doCalcNorm) {
                ...
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_THRESH_KERNEL(nrm * (mtrx0 * Y0), nrm * (mtrx3 * qubit[1U]), size_t);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_THRESH_KERNEL(nrm * (mtrx1 * qubit[1U]), nrm * (mtrx2 * Y0), size_t);
                    } else {
                        fn = NORM_THRESH_KERNEL(nrm * ((mtrx0 * Y0) + (mtrx1 * qubit[1U])),
                            nrm * ((mtrx2 * Y0) + (mtrx3 * qubit[1U])), size_t);
                    }
                } else {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_THRESH_KERNEL(mtrx0 * Y0, mtrx3 * qubit[1U], size_t);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_THRESH_KERNEL(mtrx1 * qubit[1U], mtrx2 * Y0, size_t);
                    } else {
                        fn = NORM_THRESH_KERNEL(
                            (mtrx0 * Y0) + (mtrx1 * qubit[1U]), (mtrx2 * Y0) + (mtrx3 * qubit[1U]), size_t);
                    }
                }
            } else {
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_CALC_KERNEL(nrm * (mtrx0 * Y0), nrm * (mtrx3 * qubit[1U]), size_t);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_CALC_KERNEL(nrm * (mtrx1 * qubit[1U]), nrm * (mtrx2 * Y0), size_t);
                    } else {
                        fn = NORM_CALC_KERNEL(nrm * ((mtrx0 * Y0) + (mtrx1 * qubit[1U])),
                            nrm * ((mtrx2 * Y0) + (mtrx3 * qubit[1U])), size_t);
                    }
                } else {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_CALC_KERNEL(mtrx0 * Y0, mtrx3 * qubit[1U], size_t);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_CALC_KERNEL(mtrx1 * qubit[1U], mtrx2 * Y0, size_t);
                    } else {
                        fn = NORM_CALC_KERNEL(
                            (mtrx0 * Y0) + (mtrx1 * qubit[1U]), (mtrx2 * Y0) + (mtrx3 * qubit[1U]), size_t);
                    }
    ...
    Dispatch(maxQPowerOcl >> bitCount,
        [this, mtrxS, qPowersSorted, offset1, offset2, doCalcNorm, doApplyNorm, nrm, nrm_thresh, bitCount] {
            ...
            if (!doCalcNorm) {
                ...
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_THRESH_KERNEL(nrm * (mtrx0 * Y0), nrm * (mtrx3 * qubit[1U]), bitCapInt);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_THRESH_KERNEL(nrm * (mtrx1 * qubit[1U]), nrm * (mtrx2 * Y0), bitCapInt);
                    } else {
                        fn = NORM_THRESH_KERNEL(nrm * ((mtrx0 * Y0) + (mtrx1 * qubit[1U])),
                            nrm * ((mtrx2 * Y0) + (mtrx3 * qubit[1U])), bitCapInt);
                    }
                } else {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_THRESH_KERNEL(mtrx0 * Y0, mtrx3 * qubit[1U], bitCapInt);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_THRESH_KERNEL(mtrx1 * qubit[1U], mtrx2 * Y0, bitCapInt);
                    } else {
                        fn = NORM_THRESH_KERNEL(
                            (mtrx0 * Y0) + (mtrx1 * qubit[1U]), (mtrx2 * Y0) + (mtrx3 * qubit[1U]), bitCapInt);
                    }
                }
            } else {
                if (abs(ONE_R1 - nrm) > REAL1_EPSILON) {
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_CALC_KERNEL(nrm * (mtrx0 * Y0), nrm * (mtrx3 * qubit[1U]), bitCapInt);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_CALC_KERNEL(nrm * (mtrx1 * qubit[1U]), nrm * (mtrx2 * Y0), bitCapInt);
                    } else {
                        fn = NORM_CALC_KERNEL(nrm * ((mtrx0 * Y0) + (mtrx1 * qubit[1U])),
                            nrm * ((mtrx2 * Y0) + (mtrx3 * qubit[1U])), bitCapInt);
                    }
                    ...
                    if (IS_NORM_0(mtrx1) && IS_NORM_0(mtrx2)) {
                        fn = NORM_CALC_KERNEL(mtrx0 * Y0, mtrx3 * qubit[1U], bitCapInt);
                    } else if (IS_NORM_0(mtrx0) && IS_NORM_0(mtrx3)) {
                        fn = NORM_CALC_KERNEL(mtrx1 * qubit[1U], mtrx2 * Y0, bitCapInt);
                    } else {
                        fn = NORM_CALC_KERNEL(
                            (mtrx0 * Y0) + (mtrx1 * qubit[1U]), (mtrx2 * Y0) + (mtrx3 * qubit[1U]), bitCapInt);
                    }
...
void QEngineCPU::XMask(const bitCapInt& mask)
{
    ...
    if (stateVec->is_sparse()) {
        return QInterface::XMask(mask);
    }

    Dispatch(maxQPowerOcl, [this, mask] {
        const size_t maskOcl = (size_t)(uint64_t)mask;
        const size_t otherMask = (maxQPowerOcl - 1U) ^ maskOcl;
        ParallelFunc fn = [&](const size_t& lcv, const unsigned& cpu) {
            const size_t otherRes = lcv & otherMask;
            size_t setInt = lcv & maskOcl;
            size_t resetInt = setInt ^ maskOcl;
...
void QEngineCPU::PhaseParity(real1_f radians, const bitCapInt& mask)
{
    ...
    if (stateVec->is_sparse()) {
        return QInterface::PhaseParity(radians, mask);
    }

    Dispatch(maxQPowerOcl, [this, mask, radians] {
        const size_t parityStartSize = 4U * sizeof(size_t);
        ...
        const size_t maskOcl = (size_t)(uint64_t)mask;
        const size_t otherMask = (maxQPowerOcl - 1U) ^ maskOcl;
        par_for(0U, maxQPowerOcl, [&](const size_t& lcv, const unsigned& cpu) {
            const size_t otherRes = lcv & otherMask;
            size_t setInt = lcv & maskOcl;
...
void QEngineCPU::PhaseRootNMask(bitLenInt n, const bitCapInt& mask)
{
    ...
    const real1_f radians = -PI_R1 / pow2Cpu(n - 1U);
    ...
    if (stateVec->is_sparse()) {
        return QInterface::PhaseRootNMask(n, mask);
    }

    Dispatch(maxQPowerOcl, [this, n, mask, radians] {
        const size_t maskOcl = (size_t)(uint64_t)mask;
        const size_t nPhases = pow2Cpu(n);
        par_for(0U, maxQPowerOcl, [&](const size_t& lcv, const unsigned& cpu) {
            const size_t nPhaseSteps = popCountOcl(lcv & maskOcl) % nPhases;
            if (nPhaseSteps != 0U) {
    ...
    const complex* mtrxs, const std::vector<bitCapInt>& mtrxSkipPowers, const bitCapInt& mtrxSkipValueMask)
{
    ...
    if (controls.empty()) {
        return Mtrx(mtrxs + ((size_t)(uint64_t)mtrxSkipValueMask * 4U), qubitIndex);
    }
    ...
    const size_t targetPower = pow2Cpu(qubitIndex);

    std::vector<size_t> qPowers(controls.size());
    std::transform(controls.begin(), controls.end(), qPowers.begin(), [](const bitLenInt& x) { return pow2Cpu(x); });

    std::vector<size_t> mtrxSkipPowersOcl(mtrxSkipPowers.size());
    std::transform(mtrxSkipPowers.begin(), mtrxSkipPowers.end(), mtrxSkipPowersOcl.begin(),
        [](const bitCapInt& i) { return (size_t)(uint64_t)i; });

    const size_t mtrxSkipValueMaskOcl = (size_t)(uint64_t)mtrxSkipValueMask;
    ...
    if (doNormalize && ((ONE_R1 - nrm) > FP_NORM_EPSILON)) {
        fn = [&](const size_t& lcv, const unsigned& cpu) {
            ...
            size_t i = 0U;
            size_t iHigh = offset;
            for (size_t p = 0U; p < mtrxSkipPowers.size(); ++p) {
                size_t iLow = iHigh & (mtrxSkipPowersOcl[p] - 1U);
                i |= iLow;
                iHigh = (iHigh ^ iLow) << 1U;
            }
            i |= iHigh;
...
void QEngineCPU::UniformParityRZ(const bitCapInt& mask, real1_f angle)
{
    ...
    Dispatch(maxQPowerOcl, [this, mask, angle] {
        ...
        if (stateVec->is_sparse()) {
            ...
            ParallelFunc fn = [&](const size_t& lcv, const unsigned& cpu) {
                stateVec->write(lcv,
                    stateVec->read(lcv) * ((popCountOcl(lcv & (size_t)(uint64_t)mask) & 1U) ? phaseFac : phaseFacAdj));
            };
            par_for(0U, maxQPowerOcl, fn);
        }
    });
...
void QEngineCPU::CUniformParityRZ(const std::vector<bitLenInt>& cControls, const bitCapInt& mask, real1_f angle)
{
    ...
    Dispatch(maxQPowerOcl >> cControls.size(), [this, controls, mask, angle] {
        size_t controlMask = 0U;
        std::vector<size_t> controlPowers(controls.size());
        for (size_t i = 0U; i < controls.size(); ++i) {
            controlPowers[i] = pow2Cpu(controls[i]);
        ...
        par_for_mask(0U, maxQPowerOcl, controlPowers, [&](const size_t& lcv, const unsigned& cpu) {
            stateVec->write(controlMask | lcv,
                stateVec->read(controlMask | lcv) *
                    ((popCountOcl(lcv & (size_t)(uint64_t)mask) & 1U) ? phaseFac : phaseFacAdj));
        });
    });
}
...
bitLenInt QEngineCPU::Compose(QEngineCPUPtr toCopy)
{
    ...
    const size_t nMaxQPower = pow2Cpu(nQubitCount);
    const size_t startMask = maxQPowerOcl - 1U;
    const size_t endMask = (toCopy->maxQPowerOcl - 1U) << qubitCount;
    ...
    const unsigned numCores = GetConcurrencyLevel();
    if (isSparse && (stateVec->is_sparse() || toCopy->stateVec->is_sparse())) {
        std::unique_ptr<real1[]> fidelityLoss = std::unique_ptr<real1[]>(new real1[numCores]());
        ParallelFuncSparse fn = [&](const bitCapInt& lcv, const unsigned& cpu) {
...
bitLenInt QEngineCPU::Compose(QEngineCPUPtr toCopy, bitLenInt start)
{
    ...
    const size_t nMaxQPower = pow2Cpu(nQubitCount);
    const size_t startMask = pow2MaskOcl(start);
    const size_t midMask = bitRegMaskOcl(start, oQubitCount);
    const size_t endMask = pow2MaskOcl(qubitCount + oQubitCount) & ~(startMask | midMask);
...
std::map<QInterfacePtr, bitLenInt> QEngineCPU::Compose(std::vector<QInterfacePtr> toCopy)
{
    ...
    std::vector<size_t> mask(toComposeCount);
    ...
    for (bitLenInt i = 0U; i < toComposeCount; ++i) {
        ...
        src->Finish();
        mask[i] = (src->maxQPowerOcl - 1U) << nQubitCount;
...
void QEngineCPU::DecomposeDispose(bitLenInt start, bitLenInt length, QEngineCPUPtr destination)
{
    ...
    const size_t partPower = pow2Cpu(length);
    const size_t remainderPower = pow2Cpu(nLength);
    ...
    if (destination) {
        par_for(0U, remainderPower, [&](const size_t& lcv, const unsigned& cpu) {
            size_t j = lcv & pow2MaskOcl(start);
            j |= (lcv ^ j) << length;

            for (size_t k = 0U; k < partPower; ++k) {
        ...
        par_for(0U, partPower, [&](const size_t& lcv, const unsigned& cpu) {
            const size_t startMask = pow2MaskOcl(start);
            const size_t j = lcv << start;

            for (size_t k = 0U; k < remainderPower; ++k) {
                size_t l = k & startMask;
                l |= j | ((k ^ l) << length);
...
void QEngineCPU::Dispose(bitLenInt start, bitLenInt length, const bitCapInt& disposedPerm)
{
    ...
    const size_t disposedPermOcl = (size_t)(uint64_t)disposedPerm;
    const size_t remainderPower = pow2Cpu(nLength);
    const size_t skipMask = pow2Cpu(start) - 1U;
    const size_t disposedRes = disposedPermOcl << start;
    ...
    par_for(0U, remainderPower, [&](const size_t& iHigh, const unsigned& cpu) {
        const size_t iLow = iHigh & skipMask;
        nStateVec->write(iHigh, stateVec->read(iLow | ((iHigh ^ iLow) << length) | disposedRes));
    });
...
real1_f QEngineCPU::Prob(bitLenInt qubit)
{
    ...
    const size_t qPower = pow2Cpu(qubit);
    const unsigned numCores = GetConcurrencyLevel();
...
real1_f QEngineCPU::CtrlOrAntiProb(bool controlState, bitLenInt control, bitLenInt target)
{
    ...
    const size_t qControlPower = pow2Cpu(control);
    const size_t qControlMask = controlState ? qControlPower : 0U;
    const size_t qPower = pow2Cpu(target);
    const unsigned numCores = GetConcurrencyLevel();
    std::unique_ptr<real1[]> oneChanceBuff(new real1[numCores]());
...
real1_f QEngineCPU::ProbReg(bitLenInt start, bitLenInt length, const bitCapInt& permutation)
{
    ...
    const size_t perm = ((size_t)(uint64_t)permutation) << start;
    ...
    if (stateVec->is_sparse()) {
        ...
        par_for_skip(0U, maxQPowerOcl, pow2Cpu(start), length, fn);
    }
    stateVec->isReadLocked = true;
...
real1_f QEngineCPU::ProbMask(const bitCapInt& mask, const bitCapInt& permutation)
{
    ...
    size_t v = (size_t)(uint64_t)mask; // count the number of bits set in v
    std::vector<size_t> skipPowersVec;
    while (v) {
        size_t oldV = v;
    ...
    const size_t permutationOcl = (size_t)(uint64_t)permutation;
    const unsigned num_threads = GetConcurrencyLevel();
    std::unique_ptr<real1[]> probs(new real1[num_threads]());

    par_for_mask(0U, maxQPowerOcl, skipPowersVec,
        [&](const size_t& lcv, const unsigned& cpu) { probs[cpu] += norm(stateVec->read(lcv | permutationOcl)); });
...
bitCapInt QEngineCPU::HighestProbAll()
{
    ...
    std::unique_ptr<size_t[]> bestPerms(new size_t[numCores]());
    par_for(0U, maxQPowerOcl, [&](const size_t& lcv, const unsigned& cpu) {
...
void QEngineCPU::ApplyM(const bitCapInt& regMask, const bitCapInt& result, const complex& nrm)
{
    ...
    Dispatch(maxQPowerOcl, [this, regMask, result, nrm] {
        if (stateVec->is_sparse()) {
            ...
            const size_t regMaskOcl = (size_t)(uint64_t)regMask;
            const size_t resultOcl = (size_t)(uint64_t)result;
            ParallelFunc fn = [&](const size_t& i, const unsigned& cpu) {
...
void QEngineCPU::NormalizeState(real1_f nrm_f, real1_f norm_thresh_f, real1_f phaseArg)
{
    ...
    ParallelFunc fn;
    if (norm_thresh <= ZERO_R1) {
        fn = [&](const size_t& lcv, const unsigned& cpu) { stateVec->write(lcv, cNrm * stateVec->read(lcv)); };
    } else {
        fn = [&](const size_t& lcv, const unsigned& cpu) {
